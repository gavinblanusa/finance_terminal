...
- **Tests**: FastAPI contract tests live in `tests/` (`pytest`); `pytest.ini` sets `pythonpath = app`. Run `pytest tests/` from the repo root. CI (`.github/workflows/ci.yml`) runs the suite with **`USE_OPENBB=true`** and **`USE_OPENBB=false`**, and a **`openbb-doc-registry`** job runs `scripts/verify_openbb_coverage_doc.py`. Targeted runs: `pytest tests/test_terminal_api.py` (mocks, no network/DB), `pytest tests/test_openbb_kernel.py` (OpenBB kernel/adapter mocks), `pytest tests/test_partnership_signal.py`, `pytest tests/test_factor_exposure.py` (synthetic factors / attribution), `pytest tests/test_market_warehouse.py` (optional MDW OHLCV path, Parquet fixtures). Linting: `ruff check app/`.
