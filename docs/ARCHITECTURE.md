...
| **Market Analysis** | `market_analysis_page()` | `market_data` (OHLCV, valuation, signals, profile, fundamentals, news, insider rows, competitors); `insider_service` (SEC Form 4 via EDGAR); `options_iv_term` (ATM IV term via yfinance); `market_data` may call `api_clients` | `.market_cache/` (file); `.edgar_cache/insider_{TICKER}.json`; `valuation_history`, `peer_overrides` (DB); Streamlit cache for IV term and insider rows (600s) |
...
6. **Insider transactions**: `market_data.fetch_insider_transactions()` calls `insider_service.fetch_insider_transactions_sec()` first for recent SEC Form 4 / 4-A XML filings, cached in `.edgar_cache/insider_{TICKER}.json` (6h). If SEC yields no rows, configured Finnhub is used as a fallback. The Corporate Activity section links to OpenInsider's ticker screener for manual cross-checking only; the app does not scrape OpenInsider.
7. **Options ATM IV term** (`options_iv_term.build_iv_term_structure`): Listed expiries via yfinance; ATM strike vs summary spot (or inferred); IV from call/put average when both exist. Cached `_cached_iv_term_structure(ticker, spot_key)` (600s). **Clear Cache** on Market Analysis clears this Streamlit cache.
8. **Black-Scholes panel** (`options_black_scholes.black_scholes_european`): European call/put theory prices in an expander; risk-free default from `_cached_tnx_last_percent()` (^TNX). Optional alignment to IV table presets.
9. **Competitors**: `market_data.get_competitors(ticker, sort_by, max_peers)` → FMP company screener (industry + market cap, with fallback to sector / sector+wide cap), optional TF-IDF description similarity ranking, user overrides from `peer_overrides` table. Screener result cached in `.market_cache/peers_candidates_{TICKER}.json` (6h). `clear_peers_cache(ticker)` invalidates file cache.
